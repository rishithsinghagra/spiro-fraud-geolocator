...
function propagateToleranceChanges() {
    if (jsonData) {
        jsonData.centroids.forEach(centroid => {
            if (centroid.closest_stations[0][1] < station_tolerance) {
                centroid.dynamic_name = centroid.closest_stations[0][0]
                centroid.type = "station"
            }
            else {
                centroid.dynamic_name = centroid.name
                centroid.type = "non-station"
            }
        });
...
document.getElementById("fileInput").addEventListener("change", async evt => {
    ...
    document.getElementById("fileDateTitle").textContent = isoDate;
...
function setupMap() {
    ...
    map = L.map("map", { zoomControl: false }).setView([0, 25], 4);
    ...
    centroidLayer = L.layerGroup({ pane: "overlay" }).addTo(map);
    selectedCentroidLayer = L.layerGroup({ pane: "overlay" }).addTo(map);
...
function unhighlightCentroid(id) {
    ...
    const color = c.type === "station" ? "green" : "red";
    m.setStyle({ radius: 7, fillColor: color, color, fillOpacity: 0.3 });
}

function showCentroidDetails(c) {
    const pings = jsonData.pings.filter(p => centroidLookup[p.centroid_id].dynamic_name === c.dynamic_name);
    ...
    centroidUpdateClosestStations(c.id);
}
...
function updateHourChart(pings) {
    ...
    hourChart = new Chart(document.getElementById("hourChart"), {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: useLocalHour ? `Local Hour (${detectedTimeZone})` : "UTC Hour" },
                    ticks: { display: false }
                },
...
function updateAmperageChart(pings) {
    ...
    amperageChart = new Chart(document.getElementById("amperageChart"), {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: "Amperage" },
                    ticks: { display: false }
                },
                y: { title: { display: true, text: "SOC Lost" } }
            }
...
function updateBmsChart(pings) {
    ...
    bmsChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: "BMS ID" },
                    ticks: { display: false }
                },
                y: { title: { display: true, text: "SOC Lost" } }
            },
...
function centroidUpdateClosestStations(centroid_id) {
    ...
    centroidLookup[centroid_id].closest_stations.forEach(station_pair => {
        let rough_distance = Math.round(station_pair[1] * 100000)
        text_content += `<br>`
        text_content += station_pair[0]
        text_content += `     |     ~` + rough_distance + `m`
    });
    closest_stations_subpanel.innerHTML = text_content;
}
...
function showBatteryDetails(bmsId) {
    ...
    // Display basic info
    document.getElementById("batteryDetails").innerHTML = `<b>Battery ID:</b> ${bmsId}<br><b>Total SOC Lost:</b> ${batteryPings.reduce((s, p) => s + p.soc_lost, 0)}`;
...
function updateBatteryCentroidChart(pings) {
    ...
    pings.forEach(p => {
        const cent = centroidLookup[p.centroid_id];
        if (!cent) return;
        const label = cent.dynamic_name || cent.id;
    ...
    batteryCentroidChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: "Centroid" },
                    ticks: { display: false }
                },
                y: { title: { display: true, text: "SOC Lost" } }
            },
...
function updateBatteryHourChart(pings) {
    ...
    batteryHourChart = new Chart(document.getElementById("batteryHourChart"), {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: batteryUseLocalHour ? `Local Hour (${batteryDetectedTimeZone})` : "UTC Hour" },
                    ticks: { display: false }
                },
                y: { title: { display: true, text: "SOC Lost" } }
            }
...
function exportGroupCSV(group, labelHint) {
    ...
    names.forEach(name => {
        const centroid = Object.values(centroidLookup).find(c => c.dynamic_name === name);
        if (!centroid) return;

        const pings = jsonData.pings.filter(
            p => centroidLookup[p.centroid_id]?.dynamic_name === name
        );
...
function displaySwapFlow(bms_id) {
    ...
    let tableHTML = `<b>Chronological Swap and Leakage Events</b>`;
